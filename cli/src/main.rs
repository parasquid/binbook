...
fn run_diag(cmd: DiagCommand) {
    ...
    let (port, frame, expected_opcode, timeout) = match &cmd {
        DiagCommand::Hello { port } => (port.clone(), diag_protocol::hello_request(1), Opcode::Hello, Duration::from_secs(2)),
        DiagCommand::Key { port, key } => {
            ...
            (port.clone(), diag_protocol::key_request(1, kc), Opcode::Key, Duration::from_secs(30))
        }
        DiagCommand::Page { port, action } => {
            let frame = match action {
                PageAction::Next => diag_protocol::page_action_request(1, binbook_diagnostic_protocol::PageAction::Next),
                PageAction::Previous => diag_protocol::page_action_request(1, binbook_diagnostic_protocol::PageAction::Previous),
                PageAction::First => diag_protocol::page_action_request(1, binbook_diagnostic_protocol::PageAction::First),
                PageAction::Last => diag_protocol::page_action_request(1, binbook_diagnostic_protocol::PageAction::Last),
                PageAction::Current => diag_protocol::page_action_request(1, binbook_diagnostic_protocol::PageAction::Current),
                PageAction::Goto { page } => diag_protocol::page_goto_request(1, *page),
            };
            (port.clone(), frame, Opcode::Page, Duration::from_secs(30))
        }
        DiagCommand::Status { port } => (port.clone(), diag_protocol::status_request(1), Opcode::Status, Duration::from_secs(2)),
        DiagCommand::Logs { port, since, clear } => {
            if *clear {
                (port.clone(), diag_protocol::log_clear_request(1), Opcode::LogClear, Duration::from_secs(2))
            } else {
                let cursor = since.unwrap_or(0);
                (port.clone(), diag_protocol::log_get_request(1, cursor, 496), Opcode::LogGet, Duration::from_secs(2))
            }
        }
        DiagCommand::Crash { port, clear } => {
            if *clear {
                (port.clone(), diag_protocol::crash_clear_request(1), Opcode::CrashClear, Duration::from_secs(2))
            } else {
                (port.clone(), diag_protocol::crash_get_request(1), Opcode::CrashGet, Duration::from_secs(2))
            }
        }
        DiagCommand::Probe { port, probe } => {
            ...
            (port.clone(), diag_protocol::display_probe_request(1, choice), Opcode::DisplayProbe, Duration::from_secs(60))
        }
    };
    ...
    match session.send_and_receive(&frame, expected_opcode, 1, timeout) {
        Ok(response) => {
            match diag_protocol::format_response(&response, expected_opcode, 1) {
