# Diagnostic Storage Extension Design

> **Location note:** Authored under `.omo/specs/` (planning agent sandbox).
> Belongs at `docs/specs/2026-07-01-diagnostic-storage-extension-design.md`.
...
`binbook-storage`) and the `xteink-hal` shared-bus arbiter.
...
- `binbook-fw::diag::dispatch_command` matches opcode → decodes payload → returns
  a `DispatchResult` (Response / RenderTurn / RenderPage / NoAction /
  DisplayProbe / LogGet / LogClear / CrashGet / CrashClear).

## Goals

- Upload a `.binbook` from host to the SD card over serial, with integrity
  verification and power-loss-safe (atomic rename) semantics.
...
- **Frame size bump:** `MAX_PAYLOAD_BYTES` → **4096** (`MAX_FRAME_BYTES` ~4120);
  ...
  host CLI adapts automatically. This benefits `LogGet` too (more records per
  fetch). Static firmware frame buffer grows to ~4 KB (trivial on ESP32-C3).
...
`flash` request returns `Status::Unsupported`. This avoids a future protocol v3
churn when LittleFS lands.

## Upload flow (stateless offset + pre-allocate + verify + rename)
...
1. `StoreUploadBegin { backend, name, total_size, crc32 }` → firmware creates
   `name.partial` in `/books/`, **pre-allocates** `total_size` (allocates
   clusters + sets file length in one FAT operation), and caches one write-handle
   keyed by `name` to avoid per-chunk open/close. Returns `Ok` /
   `NotFound`(no card) / `BadRequest`(bad name / too large / exists).
2. `StoreUploadWrite { backend, name, offset, data[] }` → idempotent; seeks to
   `offset` and overwrites bytes within the pre-allocated range using the cached
   handle. Frames may arrive out of order or be re-sent; each write is
   self-contained. Host pipelines multiple `StoreUploadWrite` frames.
3. `StoreUploadCommit { backend, name, expected_crc32 }` → flush, verify CRC32 of
   the written bytes, **rename `name.partial` → `name`** on match (atomic-ish on
   FAT) or delete the `.partial` on mismatch. Returns `Ok` / `Error(CrcMismatch)`.
4. `StoreAbort { backend, name }` (optional) → discard `name.partial` and drop the
   cached handle.

A per-frame CRC already protects transport; the final whole-file CRC32 (which
`binbook inspect` already computes) is the integrity check.

## `StoreList`

Returns entries from `/books/`. Each entry is **header-only and cheap**:
`{ name, file_size, page_count }`. `page_count` comes from the BinBook header
read during enumeration (A's `binbook-storage::enumerate_binbooks` validation).
Full `BookMetadata` (title/author) is a deeper read and is **not** included;
fetch on demand if C's menu later wants titles.
...
`StoreDelete { backend, name }` → FAT remove. Reject if a `.partial` upload of
the same `name` is in flight. Atomic via the `.partial` pattern (the live file is
removed only when no upload touches it).
...
pushes it (pipelined); `list` prints entries; `delete <name>` removes; `read
<name>` fetches bytes (used by the verification harness). The CLI reads
`max_frame_bytes` from `Hello` and adapts chunk sizing.

## Crate wiring (`binbook-fw`)

`diag::dispatch_command` gains new `DispatchResult` variants that route storage
opcodes to `binbook-storage` / `embedded-sd-storage` over the `xteink-hal`
shared bus (gated). The diagnostic task is the sole SD accessor in B (C's reader
will share the same arbiter). One cached write-handle per active upload name;
released on commit/abort.

## Concurrency / bus

All SD access goes through the A shared-bus mutex with display-gating. Upload
...
- **Host unit tests (no hardware):**
  ...
  - `binbook-fw` dispatch: new opcodes against a **mock `Filesystem`**,
    asserting correct `DispatchResult` variants and error statuses (bad name,
    too large, crc mismatch, delete-in-flight, no card).
- **Hardware evidence gate — the whole point of B** — uses the real
  ...
  5. `storage read nav_probe.binbook` (paginated to EOF) → host confirms
     **byte-identical** to the source, and CRC matches `binbook inspect`.
  ...
  7. Bridge into C's visual gate: re-upload, then decode page 0 through the
     display path and **webcam**-inspect it.

Capture the full serial transcript as evidence. Every CRUD path must be
exercised with discriminating preconditions (empty-before, exactly-one-after,
empty-after-delete), per the completion-evidence discipline.
...
- Whether `embedded-sdmmc` exposes a pre-allocate/`truncate`-to-size primitive,
  or whether pre-allocation is approximated by sequential allocation on first
  write.
- Exact `Unsupported` status byte value and whether `Hello` should also advertise
  which backends are implemented (a capabilities refinement).
